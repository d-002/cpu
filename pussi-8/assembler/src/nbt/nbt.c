...
void print_nbt_tree(nbt_tag_t *tag, int indentation)
{
    ...
    case NBT_TYPE_BYTE: {
        printf("%hhd", tag->payload.tag_byte.value);
        break;
    }
    case NBT_TYPE_SHORT: {
        printf("%hd", tag->payload.tag_short.value);
        break;
    }
    case NBT_TYPE_INT: {
        printf("%d", tag->payload.tag_int.value);
        break;
    }
    case NBT_TYPE_LONG: {
        printf("%ld", tag->payload.tag_long.value);
        break;
    }
    case NBT_TYPE_FLOAT: {
        printf("%f", tag->payload.tag_float.value);
        break;
    }
    case NBT_TYPE_DOUBLE: {
        printf("%f", tag->payload.tag_double.value);
        break;
    }
    case NBT_TYPE_BYTE_ARRAY: {
        ...
        {
            printf("%hhd ", tag->payload.tag_byte_array.value[i]);
        }
        break;
    }
    case NBT_TYPE_STRING: {
        printf("%s", tag->payload.tag_string.value);
        break;
    }
    case NBT_TYPE_LIST: {
        printf("\n");
        ...
        {
            print_nbt_tree(tag->payload.tag_list.value[i],
                           indentation + tag->name_size + 2);
        }
        break;
    }
    case NBT_TYPE_COMPOUND: {
        ...
        {
            print_nbt_tree(tag->payload.tag_compound.value[i],
                           indentation + tag->name_size + 2);
        }
        break;
    }
    case NBT_TYPE_INT_ARRAY: {
        ...
        {
            printf("%d ", tag->payload.tag_int_array.value[i]);
        }
        break;
    }
    case NBT_TYPE_LONG_ARRAY: {
        ...
        {
            printf("%ld ", tag->payload.tag_long_array.value[i]);
        }
        break;
    }
    default: {
...
int main()
{
    ...
    nbt_tag_t *tag_stringtest =
        nbt_new_tag_string("HELLO WORLD THIS IS A TEST STRING ÅÄÖ!",
                           strlen("HELLO WORLD THIS IS A TEST STRING ÅÄÖ!"));
    ...
    nbt_tag_t *read_test_1 =
        read_nbt_file("write_test_raw.nbt", NBT_PARSE_FLAG_USE_RAW);
    ...
    nbt_tag_t *read_test_2 =
        read_nbt_file("write_test_zlib.nbt", NBT_PARSE_FLAG_USE_ZLIB);
    ...
    nbt_tag_t *read_test_3 =
        read_nbt_file("write_test_gzip.nbt", NBT_PARSE_FLAG_USE_GZIP);
